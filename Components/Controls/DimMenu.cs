...
{
    public Dictionary<int, DimMenuItem> MenuItems { get; private set; } = [];
    public int SelectedIndex { get; private set; } = 0;
    public string SelectedLabel => MenuItems[SelectedIndex].Label;
    ...
    internal DimMenu(Rect bounds, Console console, Font font, Dictionary<string, Action> items) : base(bounds, console)
    {
        ...
        {
            MenuItems.Add(MenuItems.Count, new DimMenuItem(kvp.Key, kvp.Value));
        }
    ...
    {
        MenuItems.Add(MenuItems.Count, new DimMenuItem(label, action));
    }
